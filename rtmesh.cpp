...
//        qDebug() << line;
        ...
        if(line[0]=="f"){
            ...
            if(vt1*vt2*vt3>0){
                rtVector3d* vvt1 = objects[objectCount-1]->coords[vt1-1];
                rtVector3d* vvt2 = objects[objectCount-1]->coords[vt2-1];
                rtVector3d* vvt3 = objects[objectCount-1]->coords[vt3-1];
                objects[objectCount-1]->triangles[facesCount]->setCoords(vvt1,vvt2,vvt3);
            }
            if(vn1*vn2*vn3>0){
                rtVector3d* vvn1 = objects[objectCount-1]->normals[vn1-1];
                rtVector3d* vvn2 = objects[objectCount-1]->normals[vn2-1];
                rtVector3d* vvn3 = objects[objectCount-1]->normals[vn3-1];
                objects[objectCount-1]->triangles[facesCount]->setNormals(vvn1,vvn2,vvn3);
            }
            facesCount++;
        }
        if(line[0]=="#"){
...
rtLightIntensity rtMesh::intersect(const rtRay &ray, double &cur_distance, const QVector<rtObject *> objects, const QVector<rtAbstractLight*> &lights)
{
    ...
    while(i<triangles.size()){                 // main loop; checks every triangle for collision and stops if detected
        bool flag = false;
        double alf,bet,det;
        flag = triangles[i]->intersect(ray,cur_distance,alf,bet,det);
        if(flag){
            face=i;
            alfa=alf;
            beta=bet;
    ...
    rtVector3d* normals =  triangles[face]->getNormals();       //calculate point normals by interpolating vertex normals using barycentric coordinates
    rtVector3d norm1 = normals[0]*alfa;
    rtVector3d norm2 = normals[1]*beta;
    rtVector3d norm3 = normals[2]*delta;
    rtVector3d normal = (norm1+norm2+norm3).normalize();
    rtVector3d* coords = triangles[face]->getCoords();
    rtVector3d coord1 = coords[0]*alfa;
    rtVector3d coord2 = coords[1]*beta;
    rtVector3d coord3 = coords[2]*delta;
    rtVector3d coord = (coord1+coord2+coord3)/3;
    return (*material)(ray, hit, normal, coord, objects,lights);
}
...
bool rtMesh::rtTriangle::intersect(const rtRay &ray, double &cur_distance, double &alfa, double &beta, double &delta)
{
//    double dotRayNormal = ray.getDirection().dotProduct(normal);    // backface check. dot product of ray direction and triangle normal.
    ...
    delta= ( (i*ak_minus_jb) + (h*jc_minus_al) + (g*bl_minus_kc) )/M;
    alfa = 1-(beta+delta);
    double t =    ( (f*ak_minus_jb) + (e*jc_minus_al) + (d*bl_minus_kc) )/M;
...
void rtMesh::rtTriangle::setNormals(rtVector3d* vn1, rtVector3d* vn2, rtVector3d* vn3)
{
    ...
    this->vn2 = vn2;
    this->vn3 = vn3;
    rtVector3d v1 = *this->vn1;
    rtVector3d v2 = *this->vn2;
    rtVector3d v3 = *this->vn3;
    this->normal = ((v1+v2+v3).normalize());
}
...
rtVector3d *rtMesh::rtTriangle::getNormals() const
{
    rtVector3d vector[3] = {*vn1,*vn2,*vn3};
    return vector;
}

rtVector3d *rtMesh::rtTriangle::getVerts() const
{
    rtVector3d vector[3] = {*v1,*v2,*v3};
    return vector;
}

rtVector3d *rtMesh::rtTriangle::getCoords() const
{
    rtVector3d vector[3] = {*vt1,*vt2,*vt3};
    return vector;
}
